            ...
            <ul className={`bg-white top-14 right-0 mt-6 space-y-6 lg:absolute lg:border lg:rounded-md lg:w-52 lg:shadow-md lg:space-y-0 lg:mt-0 ${state ? '' : 'lg:hidden'}`}>
                {
                    navigation.map((item, idx) => (
                        <li key={idx}>
                            <a className="block text-gray-600 hover:text-gray-900 lg:hover:bg-gray-50 lg:p-3"
                               href={item.path}>
                                {item.title}
                            </a>
                        </li>
...
// Replace  paths with your paths
    const navigation = [
        {title: "convecatio", path: ""},
        {title: "Support", path: ""},
    ]

    const submenuNav = [
        {title: "Convocation des examens", path: ""},
        {title: "Listes des examens", path: ""},
    ]
